...
- **查词**：先查本地词典的精确词条；没有命中时，用 Free Dictionary 获取音标、词性、多义项和例句，再由 Google / 有道补充中文释义和整词翻译。整词的短释义会放在最前面，详细义项随后显示。
- **药物查询**：调用从 `drugshop` 集成的完整公开数据查询链路。
- **自动划词**：在其他 Windows 应用中用鼠标选中文本后，同时执行上面两条查询，并弹出同一个结果窗口；不是药物时明确显示“未找到该药物”。
- **快捷键取词**：在网页或其他应用选中文本后按 `Ctrl + Alt + D`，Medict 会先读取选区，再直接执行查词和药物查询；没有选区时才只打开窗口。
- **缓存与阅读**：药物结果跨重启缓存 7 天；设置里可调整结果字号，输入框、整块结果、直接释义和每个详细义项都可单独复制。
...
快捷键 `Ctrl + Alt + D` 会优先读取当前应用的选中文本并立即查询；没有读取到选区时才重新呼出已隐藏的 Medict 窗口。关闭按钮默认隐藏到系统托盘；可以在设置中彻底退出。
